<template>
  <div class="wb-cmdb-view">
    <div class="view-header glass-effect animate-fade-in-down">
      <div class="header-content">
        <h1 class="view-title">WB CMDB信息 <el-tag size="small" type="warning">TODO</el-tag></h1>
        <p class="view-subtitle">WB系统配置信息管理</p>
      </div>
...
import { getWBCMDBList, createWBCMDB, updateWBCMDB, deleteWBCMDB } from '../../api/cmdb'
import type { WBCMDBInfo, WBCMDBInfoRequest } from '../../types/cmdb'
...
const tableData = ref<WBCMDBInfo[]>([])

const formData = reactive<WBCMDBInfoRequest>({
  systemName: '',
  environment: '',
  ipAddress: '',
  port: '',
...
const dialogTitle = computed(() => currentEditId.value ? '编辑WB CMDB信息' : '新增WB CMDB信息')

const fetchData = async () => {
  loading.value = true
  try {
    const res = await getWBCMDBList({
...
const handleEdit = (row: WBCMDBInfo) => {
...
const handleDelete = async (row: WBCMDBInfo) => {
  try {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      if (currentEditId.value) {
        await updateWBCMDB(currentEditId.value, formData)
        ElMessage.success('更新成功')
      } else {
        await createWBCMDB(formData)
